...
def find(afile):
    """
        As whereis look only for executable, this find look for all file type
    """
...
OPENMVG_SFM_BIN = whereis("openMVG_main_SfMInit_ImageListing")
...
if not OPENMVG_SFM_BIN:
    OPENMVG_SFM_BIN = input("openMVG install dir ? ")
if not OPENMVS_BIN:
    OPENMVS_BIN = input("openMVS install dir ? ")
if not CAMERA_SENSOR_DB_DIRECTORY:
    CAMERA_SENSOR_DB_DIRECTORY = input("openMVG camera database (%s) dir ? " % CAMERA_SENSOR_DB_FILE)
